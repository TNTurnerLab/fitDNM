    ...
    for(i in c(1:refine)){
      ...
      while(stop == 0){
        ...
        tmp2=  tmp - (sum(lambdas *weights*exp(weights * mu.new)) / sum(lambdas*exp(weights * mu.new)))
          ...
          #break
        #}
        #print(is.na(sign.new))
        sign.new = tmp2/abs(tmp2)
        if (is.na(sign.new) == TRUE){
          mu.old = NULL
          sign.old = NULL
          return(NULL)
          stop =10
          #stop("solve_s_u:tmp2 is less than or equal to 0")
        }else if((sign.old * sign.new) <=0){
          stop=1
        }else{
          mu.old = mu.new
...
double_saddle_point_approximation = function(y,lambdas, weights){
  ...
  for(i in c((start+1):100)){
    #tmp_result2 = conditional_approximation_2(x=4,y=y,lambdas = lambdas,weights=weights) *dpois(i,sum(lambdas))
    ...
    print(abs(tmp_result2/tmp_result))
